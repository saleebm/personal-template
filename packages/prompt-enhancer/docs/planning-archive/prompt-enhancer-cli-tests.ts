...
async function main() {
  ...
  try {
    ...
    if (values.interactive && enhanced.clarifyingQuestions && enhanced.clarifyingQuestions.length > 0) {
    ...
    console.log(`Valid: ${enhanced.validation.isValid ? chalk.green('✅') : chalk.red('❌')}`);
    ...
    console.log(`Instruction: ${chalk.white(enhanced.instruction.slice(0, 100))}...`);
    ...
    if (!values.output) {
      ...
      const preview = enhancer.export(enhanced, 'markdown').split('\n').slice(0, 20).join('\n');
      console.log(preview);
      console.log(chalk.gray('─'.repeat(60)));
...
describe('PromptEnhancer E2E Tests', () => {
  ...
  describe('Validation', () => {
    ...
    it('should score prompt quality accurately', async () => {
      ...
      expect(highQuality.validation.score).toBeGreaterThan(lowQuality.validation.score);
    });

    it('should identify vague language', async () => {
      const vague = await enhancer.enhance('Do something to fix the thing somehow');
  ...
  describe('Context Analysis', () => {
    beforeEach(async () => {
      ...
      await writeFile(join(testDir, 'src', 'logger.ts'), `
      ...
      await writeFile(join(testDir, 'src', 'auth.service.ts'), `
  ...
  describe('Export Formats', () => {
    it('should export to markdown format', async () => {
      const enhanced = await enhancer.enhance('Create user authentication system');
  ...
  describe('Error Handling', () => {
    it('should handle invalid input gracefully', async () => {
      await expect(enhancer.enhance('')).rejects.toThrow('Input cannot be empty');
      await expect(enhancer.enhance(null as any)).rejects.toThrow('Input cannot be empty');
    });
    ...
    it('should handle missing prompts gracefully', async () => {
      await expect(enhancer.retrieve('non-existent-id')).rejects.toThrow('Prompt not found');
    });
  });
  ...
  describe('Performance', () => {
    ...
    it('should handle large prompts efficiently', async () => {
      const largePrompt = 'Create a comprehensive system that ' + 
    ...
    it('should enforce token limits', async () => {
      const veryLargePrompt = 'Implement '.repeat(1000) + 
                              'with detailed requirements '.repeat(1000);
