...
export class PromptEnhancerSDK {
  ...
  async enhance(raw: string | RawPromptInput): Promise<StructuredPrompt> {
    ...
    try {
      ...
      const structured = await this.structureBuilder.build(
        parsed,
        context,
        input.metadata,
      );
  ...
  private exportToYaml(prompt: StructuredPrompt): string {
    ...
    yaml.push("context:");
    yaml.push(
      `  technicalStack: [${prompt.context.technicalStack.join(", ")}]`,
    );
...
export class PromptParser {
  ...
  private extractRequirements(content: string): string[] {
    ...
    for (const sentence of sentences) {
      ...
      if (
        /\b(should|must|need|require|want|have to|ensure|make sure)\b/i.test(
          sentence,
        )
      ) {
      ...
      // Check for imperative sentences
        ...
        /^(add|create|implement|fix|update|remove|change|modify|improve)/i.test(
          sentence.trim(),
        )
      ) {
        requirements.push(sentence.trim());
