...
async function runExamples() {
  ...
  const structuredInput: RawPromptInput = {
    content: 'Implement a caching layer for the API endpoints to improve performance',
    type: 'optimization',
    metadata: {
      taskId: 'PERF-001',
  ...
  try {
    const workflowResult: EnhancedPromptResult = await enhancer.enhanceWithWorkflow(
  ...
  const workflowExamples = [
    ...
    { content: 'Clean up the authentication service', type: 'refactor' as const },
    { content: 'Document the API endpoints', type: 'documentation' as const },
    { content: 'Investigate memory leak in production', type: 'research' as const },
  ...
  for (const example of workflowExamples) {
    try {
      ...
      console.log(chalk.gray(`  Success Criteria: ${enhanced.successCriteria?.length || 0} items`));
    } catch (error) {
  ...
  console.log(chalk.cyan('\n  JSON Export:'), jsonExport.substring(0, 100) + '...');
  ...
  try {
    ...
    const updated = await enhancer.update(storedId, {
      instruction: promptToUpdate.instruction + '\n\nInclude OpenAPI specification.',
      successCriteria: [
  ...
  console.log(chalk.cyan(`  AI Service Status: ${isReady ? '✅ Ready' : '⚠️ Using Fallback'}`));
