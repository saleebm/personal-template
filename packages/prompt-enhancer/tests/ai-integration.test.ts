...
describe('AI Integration', () => {
  ...
  const hasApiKey = !!process.env['GOOGLE_API_KEY'] || !!process.env['GOOGLE_GENERATIVE_AI_API_KEY'];
  const skipTest = hasApiKey ? it : it.skip;

  skipTest('should connect to AI service', async () => {
    const sdk = new PromptEnhancerSDK();
    const ready = await sdk.isReady();
    expect(ready).toBe(true);
  }, 10000);

  skipTest('should enhance prompt with AI', async () => {
    const sdk = new PromptEnhancerSDK();
  ...
  skipTest('should provide quality scoring', async () => {
    const sdk = new PromptEnhancerSDK();
    const input = 'Help me';
  ...
  skipTest('should track token usage', async () => {
    const sdk = new PromptEnhancerSDK();
  ...
  skipTest('should detect task complexity', async () => {
    const sdk = new PromptEnhancerSDK();
    const aiService = sdk['aiService'] as any; // Access private member for testing
