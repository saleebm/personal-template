...
export class AIResponseMocker {
  ...
  async loadFixtures(fixtureDir: string = "tests/fixtures/api-payloads") {
    try {
      ...
      const googleSimple = await import(
        `../../${fixtureDir}/google/simple-feature.json`
      );
  ...
  getMockResponse(
    prompt: string,
    options?: { shouldError?: boolean; errorType?: string },
  ): any {
...
export function mockAISDK(options?: {
  ...
  const mockedGenerateObject = mock(
    async ({
      model,
      schema,
      prompt,
  ...
  // Mock generateText
  const mockedGenerateText = mock(
    async ({ model, prompt, tools, temperature, maxRetries }: any) => {
      const modelName = typeof model === "string" ? model : "unknown";
