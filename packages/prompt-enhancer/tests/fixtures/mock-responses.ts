 ...
 */
export function createMockEnhancedPrompt(overrides: Partial<StructuredPrompt> = {}): StructuredPrompt {
  return {
    id: 'test-id-' + Date.now(),
    version: '1.0.0',
 ...
 */
export function createMockAIEnhancement(overrides: Partial<AIEnhancement> = {}): AIEnhancement {
  return {
...
export const MOCK_API_RESPONSES = {
  ...
  complex: createMockAIEnhancement({
    instruction: 'Design and implement a comprehensive authentication system with OAuth2, JWT, and MFA support',
    workflowType: 'feature',
    confidenceScore: 85,
    tokenCount: 500,
    successCriteria: [
  ...
  bug: createMockAIEnhancement({
    instruction: 'Fix the memory leak in the user service causing server crashes',
    workflowType: 'bug',
    confidenceScore: 88,
    tokenCount: 150,
    successCriteria: [
