...
export const TEST_PROMPTS = {
  ...
  // Complex prompts
  complex: {
    authentication: 'Redesign the entire authentication system with OAuth2, SAML, and multi-factor authentication including biometrics',
    dataArchitecture: 'Build a scalable data pipeline that processes 1TB of data daily with real-time streaming and batch processing',
    microservices: 'Convert monolithic application to microservices with service mesh, circuit breakers, and distributed tracing'
  },
  ...
  // Context-dependent
  contextual: {
    withReferences: 'Update the function in @user-service.ts to handle the new requirements',
    withLinks: 'Implement the design from https://example.com/design-doc',
    withFiles: 'Refactor the code in src/services/auth.ts and src/controllers/user.ts',
  ...
  performance: {
    ...
    deeplyNested: JSON.stringify({ a: { b: { c: { d: { e: 'value' } } } } }, null, 2)
  }
};
...
export function generateRandomPrompt(): string {
  ...
  const features = ['logging', 'monitoring', 'caching', 'validation', 'encryption'];
  const services = ['user service', 'payment gateway', 'notification system', 'search engine'];
  const modules = ['authentication', 'authorization', 'data processing', 'file handling'];
  const qualities = ['performance', 'maintainability', 'scalability', 'security'];
  const patterns = ['singleton', 'factory', 'observer', 'strategy', 'decorator'];
  ...
  return template
    .replace('${issue}', issues[Math.floor(Math.random() * issues.length)] || 'bug')
    .replace('${component}', components[Math.floor(Math.random() * components.length)] || 'component')
    .replace('${feature}', features[Math.floor(Math.random() * features.length)] || 'feature')
    .replace('${service}', services[Math.floor(Math.random() * services.length)] || 'service')
    .replace('${module}', modules[Math.floor(Math.random() * modules.length)] || 'module')
    .replace('${quality}', qualities[Math.floor(Math.random() * qualities.length)] || 'quality')
    .replace('${pattern}', patterns[Math.floor(Math.random() * patterns.length)] || 'pattern')
    .replace('${layer}', layers[Math.floor(Math.random() * layers.length)] || 'layer')
    .replace('${problem}', problems[Math.floor(Math.random() * problems.length)] || 'problem')
    .replace('${environment}', environments[Math.floor(Math.random() * environments.length)] || 'environment');
}
