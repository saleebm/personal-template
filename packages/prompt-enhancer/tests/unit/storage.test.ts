...
import { setupTestEnvironment, createTestFile, FileSystemErrorSimulator } from '../utils/setup.js';
...
describe('PromptStorage - Comprehensive Tests', () => {
  ...
  describe('File System Operations', () => {
    ...
    it('should recover from corrupted JSON files', async () => {
      ...
      await writeFile(filePath, '{ "corrupted": json file with syntax error', 'utf-8');
    ...
    it('should prevent directory traversal attacks', async () => {
      ...
      const safeId = (maliciousPrompt.id || 'fallback').replace(/[^a-zA-Z0-9-]/g, '_');
      const date = new Date().toISOString().split('T')[0];
      const safePath = join(testEnv.tempDir, 'storage', date || 'unknown', `prompt_${safeId}.json`);
    ...
    it('should handle concurrent writes safely', async () => {
      const prompts = Array(10).fill(null).map((_, i) => 
        createMockEnhancedPrompt({ id: `concurrent-${i}` })
      );
  ...
  describe('Data Integrity', () => {
    ...
    it('should use atomic writes to prevent corruption', async () => {
      ...
      try {
        ...
        const hasTempFile = operations.some(op => 
          op.includes('.tmp') || op.includes('temp')
        );
  ...
  describe('Search and Retrieval', () => {
    it('should handle search with 1000+ saved prompts efficiently', async () => {
      // Store many prompts
      const prompts = Array(1000).fill(null).map((_, i) => 
    ...
    it('should sanitize file names to prevent injection', async () => {
      ...
      for (const id of maliciousIds) {
        ...
        try {
          ...
          // Verify saved safely
          const date = new Date().toISOString().split('T')[0];
          const storageDir = join(testEnv.tempDir, 'storage', date || 'unknown');
