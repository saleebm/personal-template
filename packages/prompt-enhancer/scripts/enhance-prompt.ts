...
if (args.length === 0 || args.includes('--help')) {
  console.log(chalk.blue('🚀 AI Dr. Prompt Enhancer CLI - Engineering Edition'));
  ...
  console.log('  --workflow    Enable workflow orchestration for complex tasks');
...
async function main() {
  ...
  try {
    ...
    const fileFlag = args.includes('--file') ? '--file' : args.includes('-f') ? '-f' : null;
    if (fileFlag) {
      ...
      try {
        ...
        if (debugMode) {
          console.log(chalk.gray(`📄 File content length: ${prompt.length} characters`));
          console.log(chalk.gray(`📄 First 100 chars: ${prompt.substring(0, 100)}...`));
        }
      } catch (error) {
    ...
    if (debugMode) {
      ...
      console.log(chalk.gray(`"${prompt.substring(0, 200)}${prompt.length > 200 ? '...' : ''}"`));
    }

    console.log(chalk.blue('🔄 Enhancing prompt...'));
    console.log(chalk.gray(`Original: ${prompt.substring(0, 100)}${prompt.length > 100 ? '...' : ''}`));
    console.log('');

    // Test if AI is ready
    const isReady = await sdk.isReady();
    if (!isReady) {
      console.log(chalk.yellow('⚠️  AI service not available, using fallback enhancement'));
    }
    ...
    if (useWorkflow) {
      ...
      console.log('');
      console.log(chalk.bold('📋 Session ID:'), result.workflow?.sessionId || 'N/A');
      console.log(chalk.bold('📊 Complexity:'), chalk.cyan(result.metadata.complexity));
      console.log(chalk.bold('🤖 Agents Required:'), result.metadata.agentsRequired ? chalk.green('Yes') : chalk.gray('No'));
      ...
      if (result.workflow?.agents && result.workflow.agents.length > 0) {
        ...
        result.workflow.agents.forEach(agent => {
          console.log(chalk.yellow(`  • ${agent.agentType} (Priority: ${agent.priority})`));
      ...
      result.workflow?.executionPlan.forEach(step => {
        const deps = step.dependencies ? ` [depends on: ${step.dependencies.join(', ')}]` : '';
        const agent = step.agent ? ` (${chalk.yellow(step.agent)})` : '';
        console.log(chalk.white(`  ${step.step}. ${step.description}${agent}${deps}`));
      });
      
      if (showTokens && result.metadata.tokenCount) {
        console.log('');
        console.log(chalk.bold('🔢 Token Usage:'), chalk.cyan(`${result.metadata.tokenCount} tokens`));
      }
      ...
      console.log('');
      console.log(chalk.bold('📈 Quality Score:'), chalk.cyan(`${result.validation.score}/100`));
      ...
      if (outputPath) {
        const exported = sdk.export(result, outputPath.endsWith('.md') ? 'markdown' : 'json');
    ...
    if (showComplexity) {
      ...
      console.log('');
      console.log(chalk.bold('🎚️  Complexity Analysis:'), chalk.cyan(complexity));
    }
    ...
    if (showAgents) {
      ...
      if (agents.length > 0) {
        agents.forEach((agent: string) => console.log(chalk.yellow(`  • ${agent}`)));
      } else {
