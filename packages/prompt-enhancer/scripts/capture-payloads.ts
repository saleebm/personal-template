...
const TEST_PROMPTS: TestPrompt[] = [
  ...
  },
  {
    ...
    prompt: 'Implement a complete authentication system with OAuth2, JWT tokens, and multi-factor authentication',
  ...
  {
    name: 'testing',
    prompt: 'Create comprehensive unit and integration tests for the payment service',
...
class PayloadCapture {
  ...
  private initializeModels() {
    ...
    if (googleKey) {
      ...
      console.log(chalk.yellow('⚠ GOOGLE_API_KEY not found - skipping Google captures'));
    }
    ...
    if (anthropicKey) {
      ...
      console.log(chalk.yellow('⚠ ANTHROPIC_API_KEY not found - skipping Anthropic captures'));
    }
  }
  ...
  async captureAll() {
    ...
    if (this.options.service === 'all' || this.options.service === 'anthropic') {
  ...
  private async captureAnthropicPayloads() {
    ...
    for (const testPrompt of TEST_PROMPTS.slice(0, 3)) { // Limit to avoid high costs
      try {
        console.log(chalk.gray(`  Capturing: ${testPrompt.name}...`));
        ...
        await this.savePayload(
          `anthropic/${testPrompt.name}.json`,
          payload
        );
...
${TEST_PROMPTS.map(p => 
  `    'google.${p.name}': google_${p.name.replace(/-/g, '_')},`
).join('\n')}
  };
