...
export class WorkflowOrchestrator {
  ...
  async orchestrateWorkflow(
    ...
    const needsAgents = complexity !== 'simple' || 
                       (enhancement.context.agentSuggestions && enhancement.context.agentSuggestions.length > 0) || 
                       false;
    ...
    const workflow: WorkflowExecution = {
      ...
      sessionId,
      prompt,
      agents: needsAgents ? this.prepareAgents(enhancement.context.agentSuggestions || [], contextFile) : undefined,
      contextFiles: [contextFile, ...this.findRelevantContextFiles(prompt.instruction)],
      executionPlan,
      status: 'pending',
      results: {
...
`;
  ...
  private prepareAgents(agentSuggestions: string[], contextFile: string): AgentCoordination[] {
  ...
  private generateAgentTask(agentType: string): string {
    const taskMap: Record<string, string> = {
      'ai-dr-workflow-orchestrator': 'Orchestrate the workflow execution and manage task dependencies',
      'ai-dr-challenger': 'Validate the implementation and identify potential issues',
      'nextjs-ui-api-engineer': 'Design and implement UI components with modern patterns',
      'principle-engineer': 'Review architecture and ensure scalability',
      'typescript-error-resolver': 'Resolve TypeScript errors and ensure type safety'
    };
  ...
  private async saveEnhancedPrompt(workflow: WorkflowExecution, enhancement: AIEnhancement): Promise<void> {
...
## Execution Plan
${workflow.executionPlan.map(step => 
...
`;
  ...
  findAgentFiles(): string[] {
    ...
    try {
      ...
      return ['ai-dr-workflow-orchestrator.md', 'ai-dr-challenger.md', 
              'nextjs-ui-api-engineer.md', 'principle-engineer.md', 
              'typescript-error-resolver.md'].filter(f => 
        existsSync(join(this.agentsDir, f))
      );
    }
  }
}