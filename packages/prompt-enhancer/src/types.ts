...
export const StructuredPromptSchema = z.object({
  ...
  handOffGuidance: z
    .string()
    .optional()
    .describe("Summary for handing off to another developer or agent"),
  openQuestions: z
    .array(z.string())
    .optional()
    .describe("Ambiguities or areas needing clarification"),
  constraintsAndNonGoals: z
    .array(z.string())
    .optional()
    .describe("Task boundaries and explicit non-goals"),
  // Agent resolution - will be defined after the schemas
  agentResolution: z
    .any()
    .optional()
    .describe("Information about resolved agent mentions"),
  discoveredReferences: z
    .array(
      z.object({
    ...
    .optional()
    .describe("URLs and library names found in the prompt"),
});
...
export const PromptEnhancerConfigSchema = z.object({
  ...
  outputDir: z
    .string()
    .default(".ai-dr/crafted")
    .describe("Directory to save enhanced prompts"),
  enableCodebaseContext: z
    .boolean()
    .default(true)
    .describe("Enable automatic codebase context analysis"),
  maxContextTokens: z
    .number()
    .default(4000)
    .describe("Maximum tokens to use for context"),
  templateLibrary: z
    .array(z.any())
    .default([])
  ...
  logLevel: z
    .enum(["silent", "error", "warn", "info", "verbose", "debug"])
    .default("error")
  ...
  model: z
    ...
    .describe("AI model to use for enhancement"),
  apiKeys: APIKeyConfigSchema.optional().describe(
    "API keys for AI services (overrides environment variables)",
  ),
});
...
export const AIEnhancementSchema = z.object({
  ...
  context: z.object({
    relevantFiles: z
      .array(z.string())
      .describe("Files that might need modification"),
    dependencies: z
      .array(z.string())
      .describe("Libraries or packages involved"),
    technicalStack: z
      .array(z.string())
    ...
    agentSuggestions: z
      .array(z.string())
      .optional()
      .describe("Suggested sub-agents for complex tasks"),
  }),
  clarifyingQuestions: z
    .array(z.string())
    .optional()
    .describe("Questions to ask if prompt is vague"),
  successCriteria: z
    .array(z.string())
  ...
  constraints: z
    .array(z.string())
    .optional()
    .describe("Limitations or requirements"),
  workflowType: WorkflowTypeSchema.describe("Type of work being requested"),
  confidenceScore: z
    .number()
    .min(0)
    .max(100)
    .describe("How confident the AI is about this enhancement"),
  ...
  orderOfSteps: z
    .array(z.string())
    .optional()
    .describe("Suggested order of execution steps"),
  tokenCount: z
    .number()
    .optional()
    .describe("Estimated token count for the enhanced prompt"),
});
...
export const WorkflowExecutionSchema = z.object({
  ...
  contextFiles: z
    .array(z.string())
    .describe("Relevant context files for the workflow"),
...
export const AgentResolutionResultSchema = z.object({
  ...
  resolvedAgents: z
    ...
    .describe("Agents found and resolved in the prompt"),
});
